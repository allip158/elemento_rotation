...
for (sample in samples) {
  keys = list(paste0(sample,'-2D'), paste0(sample,'-TO'), paste0(sample,'-GLICO'), paste0(sample,'-XE'))
  ...
  genes.use = unique(c(head(rownames(seurat.objs[[paste0(sample,'-2D')]]@hvg.info), 1000),
                       ...
                       head(rownames(seurat.objs[[paste0(sample,'-XE')]]@hvg.info), 1000)))
  ...
  genes.use = intersect(genes.use, rownames(seurat.objs[[paste0(sample,'-XE')]]@scale.data))
